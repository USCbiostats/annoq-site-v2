...
Per-chromosome TopMed and HRC r1.1 counts behind the Data > Statistics table.

**Generated by** [annoq-data-builder](https://github.com/USCbiostats/annoq-data-builder) →
`wgsa_add/merge_hrc_topmed.py` (branch `annoq-site-78-add-hrc-mapping-info`). That script appends the
`Mapped_in_HRC`, `HRC_chr_pos` and `HRC_chr_pos_ref_alt` columns to the TopMed WGSA output by
checking each TopMed variant's hg19 coordinates against the raw HRC r1.1 reference VCF, and writes
these counts as a by-product. This file is a snapshot of that run — do not hand-edit it; regenerate
it upstream.

Fields, per chromosome:
...
`mapped_Y + mapped_N + mapped_dot == topmed_rows` for every chromosome — the three are a partition
of the TopMed rows, not overlapping subsets. `src/data/chromosomeStats.test.ts` asserts this, so a
future snapshot that changes meaning fails the build rather than quietly mislabelling the table.

Consumed at build time only, by `scripts/generate-chromosome-data.mjs`.
